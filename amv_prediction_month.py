...
def train_network(net, criterion, optimizer, trainloader, testloader, 
                  experiment_name, num_epochs=300):
  """
  inputs
  ------
...
# ================================================================
   ...
   train_predictors = var1_comb[0:60,:,:,:].astype(np.float32)
   train_predictands = var1_ts[0:60].astype(np.float32)
   test_predictors = var1_comb[-20:,:,:,:].astype(np.float32)
   test_predictands = var1_ts[-20:].astype(np.float32)
#   test_predictors = var3_comb[-20:,:,:,:].astype(np.float32)
#   test_predictands = var3_ts[-20:].astype(np.float32)
...
#   plot_nino_time_series(test_predictands, predictions, '{} Predictions. \n Corr: {:3f}. \n RMSE: {:3f}.'.format(experiment_name, corr, rmse))
   ...
   plt.figure()
   plt.contourf(lon,lat,var1_comb[11,:,:,:].squeeze(), levels=np.linspace(-2,2,21),cmap='RdBu_r')
   plt.colorbar()
